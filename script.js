...
const TOTAL_BUDGET_KRW = 1_000_000; // ₩1,000,000 ~ $1,000
const KRW_PER_USD = 1000; // simple conversion (adjust if you want)

let items = [];
let archived = new Set(); // slugs for archived
...
function personFromNotes(item){
  if (item.category !== "Shopping List") return null;
  const m = /\*\*Shopping List:\s*([A-Za-z]+).*?\*\*/.exec(item.notes || "");
  return m ? m[1].toLowerCase() : null;
}
function tagClass(who){
  switch(who){
    case "yasaman": return "tag tag-yasaman";
    case "jay": return "tag tag-jay";
    case "mom": return "tag tag-mom";
    case "father": return "tag tag-father";
    case "nas": return "tag tag-nas";
    default: return "tag";
  }
}
...
async function save(){
  try {
    ...
    localStorage.setItem(STORAGE_ITEMS, JSON.stringify(items)); // fallback cache
...
async function load(){
  try {
    ...
    if (!snapshot.empty) {
      ...
      // fallback to local or default dataset
      items = JSON.parse(localStorage.getItem(STORAGE_ITEMS)) || attractions.slice();
      console.log("ℹ️ Firestore empty, using local/default data");
    }
  } catch (err) {
  ...
  items.forEach(it => { if (!("count" in it)) it.count = 1; });
...
function renderMain(){
  ...
  for(const it of list){ el.cards.appendChild(cardNode(it, false)); }
}
...
function renderArchive(){
  ...
  for(const it of list){ el.archiveCards.appendChild(cardNode(it, true)); }
}
...
function cardNode(item, isArchived){
  ...
  cb.type = "checkbox"; cb.className = "checkbox"; cb.checked = isArchived;
  cb.title = isArchived ? "Unarchive" : "Archive";
  cb.addEventListener("change", () => {
    if (cb.checked) archived.add(key); else archived.delete(key);
  ...
  title.className = "item-title"; title.textContent = item.name;

  top.append(cb, badge, title);
  ...
  `;
  const who = personFromNotes(item);
  if (who){
    const tags = document.createElement("div"); tags.className = "tags";
    const t = document.createElement("span"); t.className = tagClass(who);
    t.textContent = who[0].toUpperCase() + who.slice(1);
    tags.appendChild(t); meta.appendChild(tags);
  }
  ...
  price.className = "price"; price.textContent = formatKRW(item.priceKRW);

  const qty = document.createElement("div");
  qty.className = "qty";
  const minus = document.createElement("button"); minus.type="button"; minus.textContent="−";
  const countSpan = document.createElement("span"); countSpan.textContent = String(item.count ?? 1);
  ...
  minus.addEventListener("click", ()=> setCount((item.count||1)-1));
  plus.addEventListener("click", ()=> setCount((item.count||1)+1));
...
editBtn.addEventListener("click", () => {
  const newNode = editFormNode(item, isArchived);
  root.replaceWith(newNode); // replace the whole card, not just inner content
});
...
function editFormNode(item, isArchived){
  const root = document.createElement("article");
root.className = `card item editing ${pastelClassFor(item.name)}`;

  const form = document.createElement("form");
  form.className = "form-col";
  form.innerHTML = `
    <label><span>Category</span><input name="category" value="${escapeAttr(item.category)}" /></label>
    <label><span>Name</span><input name="name" value="${escapeAttr(item.name)}" /></label>
    <label><span>Address</span><textarea name="address">${escapeAttr(item.address)}</textarea></label>
    <label><span>Hours</span><textarea name="hours">${escapeAttr(item.hours)}</textarea></label>
    <label><span>Notes</span><textarea name="notes">${escapeAttr(item.notes || "")}</textarea></label>
    <label><span>Price (KRW)</span><input name="priceKRW" type="number" min="0" step="100" value="${Number(item.priceKRW)||0}" /></label>
    <label><span>Quantity</span><input name="count" type="number" min="1" step="1" value="${Number(item.count||1)}" /></label>
  ...
  form.addEventListener("submit", (e) => {
    ...
    const updated = {
      ...
      priceKRW: Math.max(0, Number(data.get("priceKRW")||0)),
      count: Math.max(1, Number(data.get("count")||1))
    };
    const oldKey = slug(item.name);
    const wasArchived = archived.has(oldKey);
    Object.assign(item, updated);
    const newKey = slug(item.name);
    if (wasArchived){ archived.delete(oldKey); archived.add(newKey); }
    save();
    // swap back to normal card
    root.replaceWith(cardNode(item, wasArchived));
    renderBudget();
  });
  form.querySelector("#cancelEdit").addEventListener("click", () => {
    root.replaceWith(cardNode(item, isArchived));
  });
...
function renderBudget(){
  ...
  const totalUSD = total / KRW_PER_USD;
  const spentUSD = spent / KRW_PER_USD;
  const remainUSD = remain / KRW_PER_USD;
  const pct = total > 0 ? Math.min(100, Math.round((spent/total)*100)) : 0;

  el.bTotal.textContent = `${formatKRW(total)} / ${formatUSD(totalUSD)}`;
  ...
  el.budget.classList.toggle("collapsed", state.budgetCollapsed);
  el.budgetToggle.textContent = state.budgetCollapsed ? "˄" : "˅";
}
...
function closeModal(){ el.modal.classList.remove("open"); el.modal.setAttribute("aria-hidden","true"); el.addForm.reset(); }
...
function hookAddForm(){
  el.addForm.addEventListener("submit",(e)=>{
    ...
    const it = {
      ...
      priceKRW: Math.max(0, Number(fd.get("priceKRW")||0)),
      count: Math.max(1, Number(fd.get("count")||1))
    };
    if (!it.category || !it.name || !it.address || !it.hours) return;
    items.push(it);
    save();
    closeModal();
    if (state.view !== "main") go("main");
    renderMain(); renderArchive(); renderBudget();
...
async function init(){
  ...
  // defaults for counts
  items.forEach(it => { if (!it.count || it.count < 1) it.count = 1; });

  renderAll();

  // filter
  el.category.addEventListener("change", ()=>{ state.category = el.category.value; renderMain(); renderArchive(); });

  // routing
  el.openArchive.addEventListener("click", ()=> go("archive"));
  el.backToMain.addEventListener("click", ()=> go("main"));
  ...
  // budget toggle
  el.budgetToggle.addEventListener("click", ()=>{
